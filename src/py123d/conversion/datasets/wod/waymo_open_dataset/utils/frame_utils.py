...
def convert_frame_to_dict(frame: dataset_pb2.Frame):
    ...
    range_images, camera_projection_protos, _, range_image_top_pose = parse_range_image_and_camera_projection(frame)
    ...
    for c in frame.context.laser_calibrations:
        ...
        data_dict[f"{laser_name_str}_LIDAR_EXTRINSIC"] = np.reshape(np.array(c.extrinsic.transform, np.float32), [4, 4])

        data_dict[f"{laser_name_str}_RANGE_IMAGE_FIRST_RETURN"] = first_return_cartesian_range_images[c.name].numpy()
        data_dict[f"{laser_name_str}_RANGE_IMAGE_SECOND_RETURN"] = second_return_cartesian_range_images[c.name].numpy()
    ...
    for im in frame.images:
        ...
        data_dict[f"{cam_name_str}_SDC_VELOCITY"] = np.array(
            [im.velocity.v_x, im.velocity.v_y, im.velocity.v_z, im.velocity.w_x, im.velocity.w_y, im.velocity.w_z],
            np.float32,
        )
        data_dict[f"{cam_name_str}_POSE"] = np.reshape(np.array(im.pose.transform, np.float32), (4, 4))
        data_dict[f"{cam_name_str}_POSE_TIMESTAMP"] = np.array(im.pose_timestamp, np.float32)
        data_dict[f"{cam_name_str}_ROLLING_SHUTTER_DURATION"] = np.array(im.shutter)
        data_dict[f"{cam_name_str}_CAMERA_TRIGGER_TIME"] = np.array(im.camera_trigger_time)
        data_dict[f"{cam_name_str}_CAMERA_READOUT_DONE_TIME"] = np.array(im.camera_readout_done_time)
    ...
    for c in frame.context.camera_calibrations:
        ...
        data_dict[f"{cam_name_str}_EXTRINSIC"] = np.reshape(np.array(c.extrinsic.transform, np.float32), [4, 4])
        data_dict[f"{cam_name_str}_WIDTH"] = np.array(c.width)
        data_dict[f"{cam_name_str}_HEIGHT"] = np.array(c.height)
        data_dict[f"{cam_name_str}_ROLLING_SHUTTER_DIRECTION"] = np.array(c.rolling_shutter_direction)
